...
class BagAdditionalFilesSpec extends ExportSpec {
  private def withBagAdditionalFiles[T](test: BagAdditionalFiles => T): T = {
    val tempDirectory = Files.createTempDirectory("bag-additional-files-spec")
    try {
      test(BagAdditionalFiles(tempDirectory))
    } finally {
      Using.resource(Files.walk(tempDirectory)) { paths =>
  ...
  "orderedExportProperties" should "return correctly ordered properties to be included in export for a Judgment transfer" in {
    ...
    withBagAdditionalFiles { bagAdditionalFiles =>
      val orderedProperties = bagAdditionalFiles.orderedExportProperties(Judgment)
      orderedProperties.size shouldBe expectedPropertiesOrder.size
      val propertiesOrder = orderedProperties.map(_.key)
      propertiesOrder should equal(expectedPropertiesOrder)
    }
